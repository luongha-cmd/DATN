...
public class TestScriptDH {
	@Test
	public void excute_TestCasedhg() throws Exception {

		String sPath = System.getProperty("user.dir") + "//src//main//java//data_Engine//DataTest.xlsx";
		ExcelUtils.setExcelFile(sPath, "Data_ÐH");
		int CasePass=0;
        int CaseFail=0;
        int CaseSkip=0;
        int row = ExcelUtils.getRowCount("Data_ÐH");
		for (int i = 1; i <= 70; i++) {
			
				System.out.println("Line:"+ i);
				String sActionKeyword = ExcelUtils.getCellData(i, 3);
				String locatorType = ExcelUtils.getCellData(i, 4);

				String locatorValue = ExcelUtils.getCellData(i, 5);
				String testData = ExcelUtils.getCellData(i, 6);
